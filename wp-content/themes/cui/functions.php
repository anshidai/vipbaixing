...
function get_ssl_avatar($avatar) {
   $avatar = preg_replace('/.*\/avatar\/(.*)\?s=([\d]+)&.*/','<img src="http://cd.v7v3.com/avatar/$1?s=50" class="avatar avatar-$2" height="50" width="50">',$avatar);
   return $avatar;
}
...
function git_custom_adminbar_menu($meta = TRUE) {
    ...
    $wp_admin_bar->add_menu(array(
        'id' => 'git_option',
        'title' => 'Cui主题使用文档', /* 设置链接名 */
        'href' => 'http://www.cuizl.com/bokezhuti/1347.html',
		'meta' => array(
            target => '_blank'
        )
    ));
