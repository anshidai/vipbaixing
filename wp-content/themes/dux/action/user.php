...
case "posts":
	...
	$args = array("caller_get_posts" => 1, "showposts" => 10, "paged" => $ui["paged"], "orderby" => "date", "author" => $cuid, "post_status" => $post_status);
	query_posts($args);
...
case "info":
	...
	$printr["user"] = array("regtime" => $udata->user_registered, "nickname" => $udata->display_name, "email" => $udata->user_email, "url" => $udata->user_url, "qq" => get_user_meta($cuid, "qq", true), "weixin" => get_user_meta($cuid, "weixin", true), "weibo" => get_user_meta($cuid, "weibo", true));
	break;
...
case "password.edit":
	...
	$status = wp_update_user(array("ID" => $cuid, "user_pass" => $ui["password"]));
...
case "info.edit":
	if (!$ui["nickname"] || ($ui["nickname"] && (12 < sstrlen($ui["nickname"]))) || ($ui["nickname"] && (sstrlen($ui["nickname"]) < 2))) {
	...
	if ($ui["url"] && (!preg_match("/^((http|https)\:\/\/)([a-z0-9-]{1,}.)?[a-z0-9-]{2,}.([a-z0-9-]{1,}.)?[a-z0-9]{2,}$/", $ui["url"]) || (100 < sstrlen($ui["url"])))) {
		print_r(json_encode(array("error" => 1, "msg" => "网址格式错误")));
		exit();
	}

	if ($ui["weibo"] && (!preg_match("/^((http|https)\:\/\/)([a-z0-9-]{1,}.)?[a-z0-9-]{2,}.([a-z0-9-]{1,}.)?[a-z0-9]{2,}$/", $ui["weibo"]) || (100 < sstrlen($ui["weibo"])))) {
	...
	if ($ui["weixin"] && (30 < sstrlen($ui["weixin"]))) {
	...
	if ($ui["weibo"]) {
		update_user_meta($cuid, "weibo", $ui["weibo"]);
	}

	if ($ui["weixin"]) {
		update_user_meta($cuid, "weixin", $ui["weixin"]);
	}

	if ($ui["qq"]) {
		update_user_meta($cuid, "qq", $ui["qq"]);
	}
	...
	if ($ui["email"]) {
		$datas["user_email"] = $ui["email"];
	}

	if ($ui["url"]) {
		$datas["user_url"] = $ui["url"];
	}
