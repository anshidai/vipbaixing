...
function _load_scripts()
{
	if (!is_admin()) {
		...
		$jss = array(
			"no"    => array("jquery" => $purl . "/js/libs/jquery.min.js", "bootstrap" => $purl . "/js/libs/bootstrap.min.js"),
			"baidu" => array("jquery" => "http://apps.bdimg.com/libs/jquery/1.9.1/jquery.min.js", "bootstrap" => "http://apps.bdimg.com/libs/bootstrap/3.2.0/js/bootstrap.min.js"),
			360     => array("jquery" => "http://libs.useso.com/js/jquery/1.9.1/jquery.min.js", "bootstrap" => "http://libs.useso.com/js/bootstrap/3.2.0/js/bootstrap.min.js"),
			"he"    => array("jquery" => "//code.jquery.com/jquery-1.9.1.min.js", "bootstrap" => "//maxcdn.bootstrapcdn.com/bootstrap/3.2.0/js/bootstrap.min.js")
			);
...
function _get_excerpt($limit = 286, $after = "...")
{
	$excerpt = get_the_excerpt();

	if ($limit < _new_strlen($excerpt)) {
...
function _comment_mail_notify($comment_id)
{
	...
	if (($parent_id != "") && ($spam_confirmed != "spam") && ($notify == "1")) {
		...
		$letter = (object) array("author" => trim(get_comment($parent_id)->comment_author), "post" => get_the_title($comment->comment_post_ID), "comment" => trim(get_comment($parent_id)->comment_content), "replyer" => trim($comment->comment_author), "reply" => trim($comment->comment_content), "link" => htmlspecialchars(get_comment_link($parent_id)), "sitename" => get_option("blogname"));
...
if (function_exists("register_nav_menus")) {
	register_nav_menus(array("nav" => __("网站导航", "haoui"), "topmenu" => __("顶部菜单", "haoui")));
}

if (function_exists("register_sidebar")) {
	$sidebars = array("gheader" => "公共头部", "gfooter" => "公共底部", "home" => "首页", "cat" => "分类页", "tag" => "标签页", "search" => "搜索页", "single" => "文章页");

	foreach ($sidebars as $key => $value ) {
		register_sidebar(array("name" => $value, "id" => $key, "before_widget" => "<div class=\"widget %2\$s\">", "after_widget" => "</div>", "before_title" => "<h3>", "after_title" => "</h3>"));
	}
}
